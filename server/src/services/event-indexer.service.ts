...
import { SOROBAN_RPC_URL, INDEXER_CONFIG, getPollingTargets } from '../lib/event-config.js'
 ...
 */
export async function indexEventsBatch(startLedger: number, endLedger: number): Promise<void> {
  ...
  console.log(`[indexer] Inserted ${inserted} events from ${startLedger}-${endLedger}`)
}
